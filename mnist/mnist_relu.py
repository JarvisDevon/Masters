...
def predict(params, inputs):
  ...
  for w, b in params[:-1]:
    ...
    activations = relu(outputs) #jnp.tanh(outputs)
...
def params_matmul(params, batch, const_vector):
  ...
  return jnp.dot(const_vector.T, grads_flat)+1e-6
...
if __name__ == "__main__":
  ...
  def update_Selective(params, batch, epoch):
    ...
    grads = grad(loss)(params, batch)
    reg_grads = grad(l2_regularizer)(params)
    return [(w-step_size*dw-wandb.config.selective_l2_step*jnp.where(jnp.abs(dw_choose)>0.01*jnp.max(dw_choose),0.0,dw_reg),\
             b-step_size*db-wandb.config.selective_l2_step*jnp.where(jnp.abs(db_choose)>0.01*jnp.max(db_choose),0.0,db_reg))\
  ...
  wandb.config.step_size_1 = 0.001
  wandb.config.step_size_2 = 0.001
  wandb.config.reg_step = 0.0001 
  wandb.config.l2_step = 0.00001 
  wandb.config.selective_l2_step = 0.00001 
  ...
  for train_index in range(len(wandb.config.training_names)):
      for seed in wandb.config.init_seeds:
          ...
          if wandb.config.init_index[train_index] == 1:    step_size = wandb.config.step_size_1
          elif wandb.config.init_index[train_index] == 2:  step_size = wandb.config.step_size_2
